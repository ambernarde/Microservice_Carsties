...
export default function AuctionForm({auction} : Props) {
  ...
  return (
    <form className='felx flex-col mt-3' onSubmit={handleSubmit(onSubmit)}>
              <Input label='Make' name='make' control={control} rules={{required:'Make is required'}}/>
              <Input label='Model' name='model' control={control} rules={{required:'Model is required'}}/>
              <Input label='Colo' name='color' control={control} rules={{required:'Color is required'}}/>

              <div className="grip grid-cols-2 gap-3 ">
                <Input label='Year' name='year' control={control}  type='number' rules={{required:'Year is required'}}/>
                <Input label='Mileage' name='mileage' control={control} rules={{required:'Model is required'}}/>
              </div>
              ...
              <div className="flex justify-between">
                  ...
                  <Button 
                     ...
                     disabled={!isValid}
                     type='submit'
                     outline color= 'sucess'>Submit</Button>
              </div>
     </form> 
  )
}
